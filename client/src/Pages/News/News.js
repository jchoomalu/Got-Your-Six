...
const News = () => {
  ...
  const showRecent = async () => {
    try {
      const getByDate = await list('news', 'date')
  ...
  const showTrending = async () => {
    try {
      const getByLikes = await list('news', 'likes')
  ...
  const showAll = async () => {
    try {
      const getAllNews = await list('news', null)
  ...
  return (
    <div>
      ...
      <Tabs user={user} showAll={showAll} showRecent={showRecent}
        ...
        change={change} setChange={setChange} model='news' />
    </div>
  )
}
